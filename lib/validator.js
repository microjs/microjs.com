var sizeToString = require('./strings').sizeToString
  ...
  , EXPECTED_TOLLERANCE_FRACTION = 0.5
...
var Validator = {
  ...
  , sizes: function(lib, sizes) {
      var exception = this.minifyExceptions.indexOf(lib.name) > -1
      if (!exception && Math.abs(sizes.raw - sizes.minified) <= MINIFIER_TOLLERANCE)
        return 'source seems to be already minified (link to unminified source)'
      if (!exception && sizes.raw <= MIN_RAW)
        return 'raw size is suspiciously small: ' + sizeToString(sizes.raw)
      if (!lib.tinyminify && sizes.minified <= MIN_MINIFIED)
      ...
      if (this.sizeExceptions.indexOf(lib.name) == -1 && sizes.gzipped > MAX_GZIPPED)
...
module.exports.create = function (config) {
  ...
  validator.sizeExceptions = config.sizeExceptions
  validator.minifyExceptions = config.minifyExceptions
  return validator
}
