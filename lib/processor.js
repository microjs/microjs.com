...
var Processor = {
    ...
    processLibrary: function (lib, callback) {
      if (!lib.name) {
        this.error(lib, 'library has no name, ignoring')
        callback()
      }

      if (!lib.source) {
        this.error(lib, 'has no source, ignoring')
        callback()
      }

      if (!lib.size) {
        this.error(lib, 'has no size estimate, ignoring')
        callback()
      }
  ...
  , processed: function(lib, callback, err, sizes) {
      ...
      if (insane) {
        this.error(lib, insane + ', ignoring')
        callback()
      }
      if (strange)
